 ...
 * io.jsonwebtoken.io.Decoders; import io.jsonwebtoken.security.Keys;
 * 
 * @Service public class JWTService { private String secretKey="";
 * 
 * public JWTService() { try { KeyGenerator
 * keyGen=KeyGenerator.getInstance("HmacSHA256"); SecretKey sk=
 * keyGen.generateKey(); secretKey
 * =Base64.getEncoder().encodeToString(sk.getEncoded()); } catch
 * (NoSuchAlgorithmException e) { e.printStackTrace(); } }
 * 
 * 
 * public String generatedToken(String username) { Map<String,Object> claims=new
 * HashMap<>();
 * 
 * return Jwts.builder() .claims() .add(claims) .subject(username) .issuedAt(new
 * Date(System.currentTimeMillis())) .expiration(new
 * Date(System.currentTimeMillis()+60*60*1000)) .and() .signWith(getKey())
 * .compact();
 * 
 * 
 * }
 ...
 * 
 * 
 * private Date extractExpiration(String token) { return
